...
class DataBaseProxy (object):
    ...
    def query_parks_group (self, provider, city, start, end):
        
        return self.db["parks_aggregated"].find \
                    ({"day":
                         {
                             '$gt': start,
                             '$lt': end
    ...
    def query_parks_df_aggregated (self, provider, city, start, end):
        
        books_cursor = self.query_parks_group(provider, city, start, end) 

        books_df = pd.DataFrame()

        for doc in books_cursor:
  
            s = pd.DataFrame(doc['parks'])
            books_df = pd.concat([books_df, s], ignore_index=True)           
...
#        return self.process_books_df(provider, books_df)[books_cols].replace({None:np.NaN})
    ...
    def filter_books_df_outliers (self, df):
        ...
        df['gnss_errors'] = df['distance'].apply(lambda w: (w > 0) & (w < 0.02)) 
        df['ride'] = df['distance'].apply(lambda w: (w > 0.02))
        df['two_way'] = df['distance'].apply(lambda w: (w > 0.02) & (w < 0.5))
        df['one_way'] = df['distance'].apply(lambda w: (w > 0.5))

        df['min_ride'] = df['duration'].apply(lambda w: (w > 3))
        df['max_daily'] = df['duration'].apply(lambda w: (w < 1440))
        df['short_trips'] = df['duration'].apply(lambda w: (w > 3) and (w < 80))
        df['long_trips'] = df['duration'].apply(lambda w: (w > 80) and (w < 1440))
...
#        return self.filter_df_days(parks_df, start, end)
    ...
    def query_parks_df_filtered_v3 (self, provider, city, start, end):

        books_df = self.query_parks_df_aggregated(provider, city, start, end)
#        return self.filter_books_df_outliers(self.filter_df_days(books_df, start, end))
        return books_df
